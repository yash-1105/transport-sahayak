...
- `synthesize_speech(text)` — Google Cloud Text-to-Speech, one batch `synthesize_speech` call, `AudioEncoding.LINEAR16` at 24kHz mono, with the WAV header explicitly parsed and stripped via the stdlib `wave` module (`_extract_pcm_from_wav`) before handing raw PCM16 to the frontend — the SAME rate `useVoiceDispatcher.ts`'s `PLAYBACK_SAMPLE_RATE` already expects, so the existing Gemini-Live-built playback path needed no new audio-decoding logic. `_extract_pcm_from_wav` also verifies the parsed WAV is mono/16-bit/24kHz and raises if not, rather than handing the frontend audio it can't safely play. **Originally used `AudioEncoding.PCM`** (its own proto docstring claims headerless output) — **real reported bug: a live call produced loud static with no intelligible speech**, the classic symptom of the frontend's raw-`Int16Array` playback path misinterpreting bytes that weren't actually headerless 16-bit PCM (the docstring's claim didn't hold up against the real API for this project, or the batch RPC doesn't honor `PCM` the same way). Switched to `LINEAR16` — decades-established, universally supported, unambiguously WAV-wrapped — and now parses that wrapper explicitly instead of assuming a header shape away. **If static/noise is ever reported again**, don't just re-guess the encoding: the Stage 7/8 logs report the exact audio byte count and computed duration, and `_extract_pcm_from_wav`'s own validation will now raise (surfacing as the `tts_text` fallback, not silent noise) if the channel count/sample width/frame rate don't match — check those log lines and exception first. Deterministic SSML wrapping (`_to_ssml`) inserts a pause between sentences — the LLM is never asked to produce SSML itself, matching this project's rule-first pattern. **Voice matching (real user request): the conversation and the closing briefing used to sound like two different people.** Gemini Live (`dispatcher_live.py`) is explicitly pinned to native-audio voice `Charon` (`_ENGLISH_VOICE_NAME`, env `GEMINI_LIVE_VOICE_NAME`) instead of an unset, undocumented, can-change-anytime API default (confirmed via a Google developer forum thread that the default is explicitly not guaranteed stable) — confirmed live, this pin does not break the conversation. **`ENGLISH_TTS_VOICE_NAME`'s default was FIRST set to `en-IN-Chirp3-HD-Charon`** (the identically-named voice in Chirp 3 HD — the closest achievable cross-engine match, since Chirp 3 HD and Gemini Live's native-audio voices are the same underlying named voice models) — **but this was a real reported bug: it made the ENTIRE closing briefing go silent** (no audio played at all, on a project where the conversation itself worked fine). Root cause: Chirp3-HD is a newer/preview voice tier not guaranteed enabled for every Google Cloud project, and its availability here was flagged as unverified at the time but shipped as the default anyway. `synthesize_speech`'s own error handling worked correctly (falls back to the `tts_text` on-screen-text event rather than crashing) — but "no audio played" looks identical whether the code is broken or the voice is simply unavailable, so an unverified voice ID must never be a default. **Reverted to `en-IN-Neural2-B`** (MALE, matching Charon's gender — Neural2 is long-established/universally available; this project's very first live TTS test used Neural2-D, proving Neural2 itself isn't the risk, only Chirp3-HD's *availability* was). Try `en-IN-Chirp3-HD-Charon` again only after confirming live availability (e.g. `gcloud ... texttospeech voices list`, or a real synthesis call) — never re-promote an unconfirmed voice to the default. `ENGLISH_TTS_SPEAKING_RATE` / `ENGLISH_TTS_PITCH` / `ENGLISH_TTS_TIMEOUT_S` (default 1.0 / 0.0 / 10s) are also configurable. Raises `EnglishTTSError` on any failure, including a malformed/unexpected WAV payload.
...
GEMINI_LIVE_VOICE_NAME=Charon   # pinned, not left at the API's unset/undocumented default -- see _ENGLISH_VOICE_NAME's comment; matches ENGLISH_TTS_VOICE_NAME below by design
...
ENGLISH_TTS_LANGUAGE_CODE=en-IN
ENGLISH_TTS_VOICE_NAME=en-IN-Neural2-B   # MALE, matches GEMINI_LIVE_VOICE_NAME's gender; en-IN-Chirp3-HD-Charon (same-named as Gemini Live's voice) was tried first but broke the briefing entirely -- Chirp3-HD availability was never confirmed live for this project. Don't switch back without verifying live first.
