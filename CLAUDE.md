...
- `generate_dispatch_script(gemini_client, state, services)` — builds a prompt from the SAME `dispatch_briefing.py` helpers (`_responder_facts_en`, `select_sops`, `_CLOSING_EN`) and asks Gemini Flash to write ONE natural spoken script from them (rule-first: the model phrases, it never decides content). Falls back to a deterministic plain-concatenation script (same facts/SOPs/closing, just joined directly) if Flash fails, times out, or returns nothing usable — the caller is never left with silence merely because Flash was unavailable. Model: `ENGLISH_BRIEFING_TEXT_MODEL` (default `gemini-2.5-flash`); timeout `ENGLISH_BRIEFING_FLASH_TIMEOUT_S` (default 8s); `ENGLISH_BRIEFING_FLASH_MAX_TOKENS` (default 800).
- `synthesize_speech(text)` — Google Cloud Text-to-Speech, one batch `synthesize_speech` call, `AudioEncoding.PCM` (confirmed from the installed `google-cloud-texttospeech` source to return headerless raw PCM16, unlike `LINEAR16` which wraps a WAV header) at 24kHz — the SAME rate `useVoiceDispatcher.ts`'s `PLAYBACK_SAMPLE_RATE` already expects, so the existing Gemini-Live-built playback path needed no new audio-decoding logic. Deterministic SSML wrapping (`_to_ssml`) inserts a pause between sentences — the LLM is never asked to produce SSML itself, matching this project's rule-first pattern. Voice: `ENGLISH_TTS_VOICE_NAME` (default `en-IN-Neural2-D`, a long-established GA voice family chosen specifically so it won't 400 on an unverified project — try a Chirp3-HD or Studio voice if this project's Cloud console shows them enabled, for higher quality). `ENGLISH_TTS_SPEAKING_RATE` / `ENGLISH_TTS_PITCH` / `ENGLISH_TTS_TIMEOUT_S` (default 1.0 / 0.0 / 10s) are also configurable. Raises `EnglishTTSError` on any failure.
- **NOT independently verified against a live Google Cloud TTS API call** (no live credentials in the environment this was built in) — before relying on this in production, verify live: the Cloud Text-to-Speech API is enabled for this project's service account (needs enabling separately from Vertex AI/Gemini — same service account, different API to turn on in the Cloud console), the chosen voice name exists, and `AudioEncoding.PCM` is accepted by the batch `synthesize_speech` RPC (as opposed to being restricted to the newer `streaming_synthesize` RPC only — if it's ever rejected, the documented fallback is `LINEAR16` + stripping its WAV header via the stdlib `wave` module). Also add `google-cloud-texttospeech>=2.16` to the deployed environment (added to `requirements.txt`).
