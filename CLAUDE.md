...
- **English is SEGMENTED into MANY short sequential Gemini Live turns — one micro-turn per fact/SOP-line/closing-line, never one combined turn** (`build_briefing_segments` in `dispatch_briefing.py`; `_brief_and_close` + `_send_next_briefing_segment` in `dispatcher_live.py`). **This took THREE rounds of live investigation to get right — read this before touching it again.**
  - **Round 1** (single combined turn, everything in one ~40+ second monologue): real reported bug — agent spoke the ambulance ETA and stopped; fire, towing, hospital, police, SOPs, and closing never reached.
  - **Round 2** (split into 3 segments: facts / SOPs / closing): fixed the hard-stop for a while, but the SAME bug came back at a later point — reported again, this time stopping after fire. Root cause found there: the model was also UNDER-DELIVERING each segment's own content (dropping towing/hospital/police from the facts list even when the turn didn't get cut off) — fixed with explicit anti-omission instructions ("Announce ALL 5", a self-check line, calling out the easy-to-drop callback line by name). Live-tested 4/4 clean at the time.
  - **Round 3**: reported a THIRD time — this time stopping after towing, even with Round 2's anti-omission instructions in place and the user having explicitly verified the Railway deployment was current. **Live-tested the actual hypothesis this time** (does a single ~5-sentence facts turn ever fail on its own?) with a real Gemini Live session, including a real ~25s idle gap injected to match `_brief_and_close`'s `dispatch_update` wait — 6/6 clean. So a single short-ish turn is NOT provably unsafe in an isolated/short-context test, yet real calls kept failing partway through it anyway. Conclusion: Gemini Live's native-audio generation reliability degrades with how much a REAL call has already accumulated in a way isolated testing cannot reproduce — a live call's context is raw AUDIO in both directions, far more token-dense per second than the equivalent text, so even a conversationally-short real call can carry a much larger context footprint than any text-based simulation of it. Each of the first two fixes bought a little more content by shortening turns somewhat; neither was conservative enough. **The only defensible fix at that point was maximum granularity**: `build_briefing_segments` now returns ONE micro-turn per responder fact, ONE per SOP line, ONE per closing line (typically 10-14 segments for a real incident) — never more than a single short sentence asked of the model in any one turn, so there is nothing left within a turn that COULD be dropped or cut short. Live-tested 6/6 clean after this change (vs. the earlier 3-segment design's ~4-5/6). **If this bug is reported a fourth time, do not shorten segments further as a first guess — get Railway's actual logs for that call first**; below some point the content itself can't be split any smaller, and the next lever has to be something else entirely (a different model, a different delivery mechanism, or accepting a lower reliability ceiling for Gemini Live native-audio and defaulting the browser to show `agentText`-style on-screen text for this phase too, mirroring Hindi's TTS-failure fallback).
  - **Round 4 (2026-07) — ACTUAL ROOT CAUSE FOUND, in the session lifecycle, not in generation or prompts.** Reported a fourth time (stopped after the towing ETA; hospital, police, SOPs, follow-up, and closing never spoken). A full implementation audit found it: `_run_live_session` returned `"ended"` whenever `state.submitted` was true — which made the ENTIRE reconnect machinery (the very budget raised to 4 attempts + exponential backoff in an earlier round!) **structurally unreachable for the whole post-submission phase**. Any Gemini Live session death mid-briefing — connection lifetime limits, `GoAway` (which the code never read or even logged), transient stream failures, all likeliest at the END of a long call, which is exactly where the briefing sits — silently ended the call: `run()` returned, `app.py` closed the socket, and the frontend's `ws.onclose` treats any post-submission close as a normal call end (`submittedRef`, by design). This is why rounds 1–3's segment-granularity work only ever "bought a little more content": it changed WHERE the session death landed in the sequence, never prevented the death or recovered from it. Fixes (all `dispatcher_live.py`; Hindi untouched — its pipeline holds no long-lived Gemini Live session that can die): (1) `"ended"` is now returned only when `_call_over` (every segment delivered + `call_complete` sent) or the browser hung up; a post-submission session death returns `"reconnect"`. (2) The briefing RESUMES across reconnects: `_current_briefing_segment` tracks the in-flight segment (cleared only when its own `turn_complete` arrives), `_reconnect_kickoff` re-delivers it in full on the fresh session (repeating one short line beats losing it), and the pump then drives the remaining queue as normal — the briefing machinery now always targets `self._live_session` at call time, never a session object captured at spawn (which would send into the dead one). If the death happened before delivery started, the resume kickoff is a holding line ("report registered, still checking, stay on the line"). (3) The stall failsafe no longer force-ends the call on 15s of silence (that silently dropped every undelivered segment — a second path to the same symptom): it closes the wedged session so the same reconnect-resume path recovers it; termination is still guaranteed (each stall burns a reconnect; on budget exhaustion `run()` errors out and cancels the briefing task). (4) `GoAway` is now logged with `time_left`, and INFO logs cover every briefing lifecycle step (dispatch_update received, briefing built with N segments, segment n/N sent / turn complete, call_complete, pump/session end reasons with submitted+call_over flags) plus browser-console logs (`[dispatcher]` — ws close code + submitted flag, call_complete drain time, backend errors) — if a call ever stops early again, Railway logs will now name the exact termination reason instead of requiring another blind round. Segment granularity was deliberately left as round 3 set it — micro-turns are harmless and each one now retries independently across reconnects. Regression tests in `tests.py`: mid-briefing session death → `"reconnect"` never `"ended"`; post-briefing session end → `"ended"`; resume kickoff replays the in-flight segment idempotently (double-death safe); pre-delivery death → holding line, never the greeting; genuine stall closes the session and never sends `call_complete` itself.
  - Total wall-clock cost of full granular delivery: **measured ~29s** for a 14-segment sequence (5 facts + 3 SOPs + 6 closing lines) — each segment is its own Gemini round trip, so more segments means more accumulated latency, not just more reliability. This is a deliberate tradeoff: a caller waiting an extra 10-15s for a complete safety briefing is better than a fast one that silently cuts off, but if this ever needs to get faster again, that's real tension against the reliability fix above, not a free win.
  - **Delivery mechanics** (unchanged in shape across all 3 rounds, only the CONTENT granularity changed): `_brief_and_close` (spawned at the first `turn_complete` after submit, after waiting on `dispatch_update` per `DISPATCH_BRIEFING_WAIT_S`) sends only the FIRST segment and then polls a stall failsafe. The PUMP (`_pump_gemini_to_client`) — the Gemini Live session's sole reader/sender, same single-reader discipline the Hindi `SaarasStream` also requires — sends each NEXT segment itself as the previous one's `turn_complete` arrives (`self._briefing_segments`, a list popped one at a time), and only calls the call over once the list is empty. This coordination is deliberately NOT split across two concurrent readers of `live_session.receive()` (`_brief_and_close` only ever *sends*, never reads, after the first segment) — an earlier draft had `_brief_and_close` try to orchestrate the whole sequence itself while awaiting a cross-task signal, which cannot work: once it decides the call is over, the pump — still blocked inside `receive()` — has no way to notice, since nothing is left to be received. This generic, list-driven design meant going from 3 segments to 10-14 needed ZERO changes to `dispatcher_live.py`'s orchestration code — only `build_briefing_segments`'s content generation changed.
  - **Stall failsafe** (`DISPATCH_BRIEFING_STALL_TIMEOUT_S`, default 15s = time since the last audio chunk, refreshed to the CURRENT segment's start each time the pump sends the next one) — not a total-length budget. Stays per-segment regardless of segment count, so a genuinely wedged session is still caught without needing to guess a total-duration budget for a sequence whose length now varies a lot by incident (more flags selected = more SOP segments = more facts if more services are relevant). Regression-tested in `tests.py` with a simulated ~20s of continuously-arriving chunks (must never cut off), a genuine no-audio stall (must still end, no hang), and a direct test of the segment-popping sequence for an arbitrary-length list.
  - **Anti-omission instructions** (from round 2, still present and still needed even at 1-item-per-turn — a turn CAN still ask its own unwanted question or add unrequested content): every non-final segment explicitly says it's the ONLY point in that turn, forbids adding anything from later in the sequence, and forbids asking a question or saying goodbye early; only the LAST segment is told the call ends here. Regression-tested in `tests.py`.
  - Hindi has no equivalent risk and was NOT changed at any point (`build_briefing_instruction`, still one combined string): its text is fully generated up front by plain `generate_content`, then handed to Bulbul TTS as one already-complete string with its own internal chunked streaming — there's no live per-turn audio-generation step that could stop partway, unlike Gemini Live which generates audio directly, turn by turn.
  - **WebSocket keepalive (`_keepalive` / `_KEEPALIVE_INTERVAL_S`, shared by both dispatchers).** A real reported bug from round 3's investigation, found while auditing the frontend at the user's request: the call would sometimes go silent mid-briefing with NO error shown at all — not a spoken cutoff, a hard connection drop, distinct from the generation-reliability issue above. Root cause: `/ws/dispatcher` had no application-level keepalive of any kind, and the post-submission phase has genuinely idle multi-second stretches with zero bytes on the wire in either direction — waiting on the browser's `dispatch_update` (up to `DISPATCH_BRIEFING_WAIT_S`), and between each briefing segment while Gemini generates the next one's audio (nothing streams during that generation latency). More segments means more such gaps, not fewer. If Railway or any proxy in the path has an idle-connection timeout, a long enough silent gap gets the socket closed out from under the call — which `useVoiceDispatcher.ts`'s `ws.onclose` handler treats, once `submittedRef.current` is true, as a normal call end (`stop()`, no reconnect, no error surfaced) — i.e. exactly "the agent just stops talking" with nothing on screen to explain why; the frontend audit found no OTHER blocking logic there, and this silent-stop-on-close is by design for the genuine "briefing finished, backend closed cleanly" case, so the real fix is preventing the spurious close, not changing how the frontend reacts to one. Fixed with a periodic lightweight `{"type":"keepalive"}` frame sent for the WHOLE call, frequently enough (`DISPATCHER_KEEPALIVE_INTERVAL_S`, default 10s) that no idle gap this call can ever produce gets close to a plausible proxy timeout. The frontend needed no change — unrecognized event types already no-op via `handleServerEvent`'s `default` case. Applied to BOTH dispatchers (Hindi inherits `_keepalive()` unchanged and spawns it in its own `run()`) since the underlying risk is infrastructure-level, not language-specific. Regression-tested in `tests.py` (fires periodically, cancels cleanly with no hang).
